...
method=("" "select" "quick" "heap" "insert" "merge" "select" "heap")
...
grade=$baseGrade
for casenum in `seq 1 1 $casecount`; do
    timeout -k $timeout $timeout ./$filename input=input$casenum.txt attributes=attributes$casenum.txt output=output$casenum.txt sort=${method[$casenum]} 1>$casenum.stdout 2>$casenum.stderr
    if [ $? -ne 0 ]; then
